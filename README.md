...
A lightweight **Windows 11** system-tray utility that captures your current **display layout + audio device** as a named `Profile` and binds to a **global hotkey**, enabling switching display and sound to a with one keypress.
...
> [!TIP]
> **Display Selector** makes changes **one hotkey**:
...
The app lives in the system tray. **Right-click (or left-click) the tray icon** for the menu:
...
Requires the **.NET 10 SDK**. (Building the installer additionally needs **Inno Setup 6** on `PATH` or in its default location.)
...
Icons generated at [recraft.ai](https://www.recraft.ai/). See [assets/image-notes.md](assets/image-notes.md).
